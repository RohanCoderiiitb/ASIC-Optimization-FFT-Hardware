...
def _decode_objectives(obj_row, fft_size):
    """
    Decode a raw 3-element objective vector into physical quantities.
    SQNR removed — no iverilog available.

    Returns dict:
        power_W, area_luts, norm_latency, crit_delay_ns, meets_timing
    """
    power_w      = obj_row[0] / WEIGHT_POWER
    area_luts    = obj_row[1] / WEIGHT_AREA
    norm_latency = obj_row[2] / WEIGHT_LATENCY
...
def plot_pareto_front(pareto_objectives, fft_size, results_subdir, feasible=True):
    """
    Save two PNG files per FFT run (SQNR removed):

      pareto_2d_fft{N}.png      — 3-panel grid: Power×Area, Power×Delay, Area×Delay
      pareto_latency_fft{N}.png — dedicated 2-panel latency dashboard
    """
    ...
    norm_latency = obj[:, 2] / WEIGHT_LATENCY
    ...
    # Reference line on every delay axis
    for ax in axes[1:]:
        ylo, yhi = ax.get_ylim()
    ...
    plt.close(fig)
    log_message(f"2-D Pareto plot (3 panels) saved → {path_2d}")

    # ── Figure 2: dedicated latency dashboard ────────────────────────────
    ...
    # Panel B: crit delay vs power (bubble size ∝ area)
    ax_cpd = axes_lat[1]
    ...
    ax_cpd.set_title("Crit Delay vs Power\n(bubble size ∝ Area)", fontsize=11)
    ax_cpd.grid(True, linestyle='--', alpha=0.4)

    from matplotlib.lines import Line2D
    legend_handles = [
        Line2D([0], [0], marker='o', color='w',
               markerfacecolor=_TIMING_OK_COLOR, markersize=9, label='Meets timing'),
        Line2D([0], [0], marker='X', color='w',
               markerfacecolor=_TIMING_BAD_COLOR, markersize=9, label='Violates timing'),
    ]
...
def save_optimization_results(result, callback, fft_size):
    ...
    Persist all artefacts for one FFT run:
      ...
      • pareto_2d_fft{N}.png               ← 3-panel pairwise (timing-colour-coded)
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write(f"Objectives            : {OBJECTIVES}  "
                f"(Power, Area, Critical-Path Delay)  [SQNR disabled]\n")
        ...
        if n_sol == 0:
            ...
            hdr = (f"{'ID':<5} {'Power(mW)':<12} {'Area(µm²)':<14} "
                   f"{'NormLat':<10} {'CritDelay(ns)':<15} {'MeetsTiming':<12}")
            f.write(hdr + "\n")
            f.write('-' * len(hdr) + '\n')
...
def run_optimization_for_fft_size(fft_size):
    ...
    log_message(f"  Objectives      : {OBJECTIVES}  (Power, Area, CritDelay)  [SQNR disabled]")
...
def generate_comprehensive_summary(all_results):
    ...
    sizes      = []
    best_power, best_area, best_delay = [], [], []
    ...
    if sizes:
        ...
        metrics = [
            (best_power, "Min Power (mW)",           _OBJ_COLORS['power'],   axes[0], False),
            (best_area,  "Min Area (µm²)",            _OBJ_COLORS['area'],    axes[1], False),
            (best_delay, "Min Crit Path Delay (ns)",  _OBJ_COLORS['latency'], axes[2], True),
        ]
        ...
        plt.close(fig)
        log_message(f"3-metric comparison plot → {comp_plot}")
...
def quick_test():
    ...
    run_optimization_for_fft_size(fft_size=8)
