...
MEMORY EXTERNALIZED:
The physical memory arrays (mixed_memory_unified) have been removed from the 
core to speed up synthesis. Instead, the core and top modules expose a direct 
SRAM interface to the testbench. The core still fully controls all memory 
arbitration, bank selection, and multiplexing internally.
...
class FFTTemplateGenerator:
    ...
    def __init__(self, fft_size):
        ...
        # Hardcoded to 11 to match original baseline AGU/Memory parameters
        self.addr_width        = 11
...
module {core_module_name} #(
    ...
    // -------------------------------------------------------------------------
    // External RAM interface (Routes to testbench)
    // -------------------------------------------------------------------------
    output wire                  mem_bank_sel,
    output wire [ADDR_WIDTH-1:0] mem_rd_addr,
    output wire                  mem_rd_prec,
    input  wire [15:0]           mem_rd_data,
    output wire                  mem_wr_en,
    output wire [ADDR_WIDTH-1:0] mem_wr_addr,
    output wire [23:0]           mem_wr_data
);
...
{prec_mux}
    ...
    twiddle_factor_unified_synthesizable #(
        .MAX_N     ({MAXn}),
        .ADDR_WIDTH(ADDR_WIDTH)
    ) twiddle_rom (
        .clk        (clk),
    ...
    // =========================================================================
    // Memory signals (Routed to external RAM ports)
    // =========================================================================
    // bank_sel: during FFT computation the core owns it;
    //           during IDLE (load) and external read the top drives ext_bank_sel.
    reg  fft_bank_sel;
    assign mem_bank_sel = (state == IDLE || ext_reading) ? ext_bank_sel : fft_bank_sel;

    // Read address mux: external read takes priority
    assign mem_rd_addr = ext_reading ? ext_rd_addr :
                         (state == READ_A) ? idx_a :
                         (state == READ_B) ? idx_b : {aw}'d0;

    assign mem_rd_prec = cur_rd_prec;

    // Write mux: external load takes priority; otherwise butterfly results
    assign mem_wr_en   = ext_wr_en ? 1'b1 :
                         (state == WRITE_X || state == WRITE_Y) ? 1'b1 : 1'b0;

    assign mem_wr_addr = ext_wr_en ? ext_wr_addr :
                         (state == WRITE_X) ? idx_a :
                         (state == WRITE_Y) ? idx_b : {aw}'d0;

    wire [23:0] X_wr_24, Y_wr_24;

    assign mem_wr_data = ext_wr_en ? ext_wr_data :
                         (state == WRITE_X) ? X_wr_24 :
                         (state == WRITE_Y) ? Y_wr_24 : 24'd0;

    // Wire external RAM read data directly into the core datapath
    wire [15:0] rd_data_16 = mem_rd_data;
...
module {top_module_name} (
    ...
    // -------------------------------------------------------------------------
    // External RAM interface (Routes core signals out to Testbench)
    // -------------------------------------------------------------------------
    output wire                  mem_bank_sel,
    ...
    input  wire [15:0]           mem_rd_data,
    output wire                  mem_wr_en,
    output wire [{aw}-1:0]       mem_wr_addr,
    output wire [23:0]           mem_wr_data
);
